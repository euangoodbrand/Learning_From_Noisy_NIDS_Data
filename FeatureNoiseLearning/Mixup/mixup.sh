...
do
  ...
  do
    ...
    for mult_noise_level in "${feature_mult_noise_levels[@]}"; do
        CUDA_LAUNCH_BLOCKING=1 python mixup.py --dataset BODMAS --model_type ${model_type} --data_augmentation none --feature_mult_noise_level ${mult_noise_level} --imbalance_ratio 0 --seed ${seed} --num_workers ${num_workers} --result_dir results/experiment_2$
    done
