...
do
  ...
  do
    ...
    for noise_rate in "${noise_rates[@]}"; do
      CUDA_LAUNCH_BLOCKING=1 python generalisedCrossEntropy.py \
    ...
    # Experiment 2: Label Noise and Imbalance
    for noise_rate in "${noise_rates[@]}"; do
      for imbalance_ratio in "${imbalance_ratios[@]}"; do
        CUDA_LAUNCH_BLOCKING=1 python generalisedCrossEntropy.py \
          ...
          --seed ${seed} \
          --num_workers ${num_workers} \
          --result_dir results/final_experiments/exp2_label_noise_imbalance
      done
    done

    # Experiment 3: Feature Noise, Label Noise, and Imbalance
    for noise_rate in "${noise_rates[@]}"; do
      for imbalance_ratio in "${imbalance_ratios[@]}"; do
        for feature_noise in "${feature_noise_levels[@]}"; do
          CUDA_LAUNCH_BLOCKING=1 python generalisedCrossEntropy.py \
            ...
            --result_dir results/final_experiments/exp3_feature_label_noise_imbalance
        done
      done
    done
