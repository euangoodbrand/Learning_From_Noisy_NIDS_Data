...
do
  ...
  do
    ...
    for add_noise_level in "${feature_add_noise_levels[@]}"; do
      CUDA_LAUNCH_BLOCKING=1 python baseline.py --dataset windows_pe_real --model_type ${model_type} --data_augmentation none --noise_rate 0 --noise_type uniform --imbalance_ratio 0 --seed ${seed} --num_workers ${num_workers} --feature_add_noise_level ${add_noise_level} --weight_decay 0.01 --result_dir results/experiment_14$
    done
    ...
    for add_noise_level in "${feature_add_noise_levels[@]}"; do
        for imbalance_ratio in "${imbalance_ratios[@]}"; do
            for weight_resampling in "${weight_resamplings[@]}"; do
                CUDA_LAUNCH_BLOCKING=1 python baseline.py --dataset windows_pe_real --model_type ${model_type} --data_augmentation none --noise_rate 0 --noise_type uniform --imbalance_ratio ${imbalance_ratio} --seed ${seed} --num_workers ${num_workers} --feature_add_noise_level ${add_noise_level} --weight_decay 0.01 --weight_resampling ${weight_resampling} --result_dir results/experiment_19 --num_runs ${num_runs}
            done
        done
    done
    ...
    for mult_noise_level in "${feature_mult_noise_levels[@]}"; do
        for imbalance_ratio in "${imbalance_ratios[@]}"; do
            for weight_resampling in "${weight_resamplings[@]}"; do
                CUDA_LAUNCH_BLOCKING=1 python baseline.py --dataset windows_pe_real --model_type ${model_type} --data_augmentation none --noise_rate 0 --noise_type uniform --imbalance_ratio ${imbalance_ratio} --seed ${seed} --num_workers ${num_workers} --feature_mult_noise_level ${mult_noise_level} --weight_decay 0.01 --weight_resampling ${weight_resampling} --result_dir results/experiment_20 --num_runs ${num_runs}
            done
        done
    done
    ...
    for add_noise_level in "${feature_add_noise_levels[@]}"; do
        for mult_noise_level in "${feature_mult_noise_levels[@]}"; do
            for imbalance_ratio in "${imbalance_ratios[@]}"; do
                for weight_resampling in "${weight_resamplings[@]}"; do
                    CUDA_LAUNCH_BLOCKING=1 python baseline.py --dataset windows_pe_real --model_type ${model_type} --data_augmentation none --noise_rate 0 --noise_type uniform --imbalance_ratio ${imbalance_ratio} --seed ${seed} --num_workers ${num_workers} --feature_add_noise_level ${add_noise_level} --feature_mult_noise_level ${mult_noise_level} --weight_decay 0.01 --weight_resampling ${weight_resampling} --result_dir results/experiment_21 --num_runs ${num_runs}
                done
            done
        done
    done
    ...
    for noise_rate in "${noise_rates[@]}"; do
        for mult_noise_level in "${feature_mult_noise_levels[@]}"; do
            for imbalance_ratio in "${imbalance_ratios[@]}"; do
                for weight_resampling in "${weight_resamplings[@]}"; do
                    CUDA_LAUNCH_BLOCKING=1 python baseline.py --dataset windows_pe_real --model_type ${model_type} --data_augmentation none --noise_rate ${noise_rate} --noise_type uniform --imbalance_ratio ${imbalance_ratio} --seed ${seed} --num_workers ${num_workers} --feature_mult_noise_level ${mult_noise_level} --weight_decay 0.01 --weight_resampling ${weight_resampling} --result_dir results/experiment_23 --num_runs ${num_runs}
                done
            done
        done
    done
